...
-- Columns haveing NULL values : Country, worlwide_gross_income, languages and production_company 
...
-- In the year 2019, a total of 1059 movies were created in either the United States or India.
...
SELECT g.genre, COUNT(DISTINCT m.id) AS number_of_movies
FROM genre g
...
-- 4285 Drama movies were produced in general and are the highest amongst all genres.
...
-- Thriller has rank=3 and film count number of 1484
...
-- Finding the rank of each film based on its average rating
-- Using the LIMIT clause, display the top ten films.
...
FROM movie m
 ...
 -- The top ten films may also be shown using the WHERE caluse with CTE.
...
AS (SELECT production_company,
           COUNT(movie_id) AS movie_count,
           RANK() OVER (ORDER BY COUNT(movie_id) DESC) AS prod_campany_rank
    FROM ratings r
        INNER JOIN movie m
            ON m.id = r.movie_id
...
-- The top three genres in March 2017 in the United States were drama, comedy, and action, with over 1,000 votes each.
-- 24 drama films were released in the United States in March 2017 and received over 1,000 votes.
...
GROUP BY country;
-- German movies received the highest number of votes campare to Italian movies
...
FROM director_mapping AS d  
   ...
   INNER JOIN genre AS g2 ON g2.movie_id = d.movie_id 
   INNER JOIN top_three_genre AS t3g on t3g.genre = g2.genre
   INNER JOIN ratings as r2 ON r2.movie_id = d.movie_id
...
WITH top_actress AS (
   ...
   FROM
      ...
      INNER JOIN ratings AS r ON m.id = r.movie_id 
   WHERE category = 'actress' AND m.country LIKE '%India%'  AND languages LIKE '%hindi%' 
   GROUP BY n.name 
...
-- Taapsee Pannu, Kriti Sanon, Divya Dutta, Shraddha Kapoor, Kriti Kharbanda are Top five actresses in Hindi movies released in India based on their average ratings 
...
WITH thriller_movies_list AS 
(
   SELECT DISTINCT m.title, avg_rating 
   FROM movie AS m 
      INNER JOIN genre AS g ON g.movie_id = m.id 
      INNER JOIN ratings AS r ON r.movie_id = m.id 
...
WITH top_genres AS (SELECT
      genre,
      COUNT(m.id) AS movie_count,
      RANK()OVER(ORDER BY COUNT(m.id)) AS genre_rank 
   FROM movie AS m 
      INNER JOIN genre AS g ON g.movie_id = m.id 
      INNER JOIN ratings AS r ON r.movie_id = m.id 
   WHERE avg_rating > 8 
...
WITH production_company_list AS (
   SELECT production_company, COUNT(*) AS movie_count, languages 
...
FROM production_company_list LIMIT 2;


-- The top two production houses that have produced the highest number of hits among multilingual movies Star Cinema and Twentieth Century Fox.
...
FROM actress_rank_list LIMIT 3;
...
WITH next_date_published_summary AS (
   SELECT  d.name_id, n.name, d.movie_id, duration, 
